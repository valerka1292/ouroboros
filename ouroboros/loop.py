...
def _estimate_cost(model: str, prompt_tokens: int, completion_tokens: int,
    ...
    """Cost estimation is disabled in unified mode (requires pricing API)."""
    return 0.0
...
def run_llm_loop(
    ...
    try:
        MAX_ROUNDS = max(1, int(os.environ.get("LLM_MAX_ROUNDS") or os.environ.get("OUROBOROS_MAX_ROUNDS", "200")))
    except (ValueError, TypeError):
        MAX_ROUNDS = 200
        log.warning("Invalid LLM_MAX_ROUNDS, defaulting to 200")
    round_idx = 0
    try:
        while True:
            round_idx += 1
